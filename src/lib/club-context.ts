...
export async function assertEntryBelongsToClub(entryId:string,clubId:string){if(!await exists("SELECT EXISTS(SELECT 1 FROM entries e JOIN shows s ON s.id=e.show_id WHERE e.id=$1 AND s.club_id=$2) AS exists",[entryId,clubId]))throw new Error("Datensatz nicht gefunden oder kein Zugriff.")}

...
export default async function RootLayout({children}:{children:React.ReactNode}){const session=await getViewerSession();const viewer=session?{clubName:session.club.name,email:session.email,isOperator:session.isOperator}:null;return <html lang="de"><body><Sidebar viewer={viewer}/><main className={session?"main":"main public-main"}>{children}</main></body></html>}
